...
export default function Home() {
	...
	const mintCryptoDevToken = async (amount) => {
		try {
			...
			await tx.wait();
			setLoading(false);
			window.alert("Sucessfully minted Skalez Inc Tokens");
	...
	const claimCryptoDevTokens = async () => {
		try {
			...
			await tx.wait();
			setLoading(false);
			window.alert("Sucessfully claimed Skalez Inc Tokens");
			await getBalanceOfCryptoDevTokens();
			await getTotalTokensMinted();
			await getTokensToBeClaimed();
		} catch (err) {
	...
	return (
		<div>
			<Head>
				<title>Crypto Devs (Skalez) </title>
				<meta name="description" content="ICO-Dapp" />
				<link rel="icon" href="/favicon.ico" />
			</Head>
			<div className={styles.main}>
				<div>
					<h1 className={styles.title}>Welcome to Skalez Inc ICO!</h1>
					<div className={styles.description}>
						You can claim or mint Skalez Inc tokens here
					</div>
					{walletConnected ? (
						<div>
							<div className={styles.description}>
